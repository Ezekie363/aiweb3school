...
::: info 相关 topic
- [Uniswap: How Uniswap Works](https://developers.uniswap.org/docs/get-started/concepts/how-uniswap-works)：理解 AMM、流动性池和 swap 的基础机制。
:::
...
1. 选择一个公开的 swap 交易哈希。
2. 在区块浏览器里查看输入 token、输出 token、路由、滑点和手续费。
3. 找到交易涉及的池子或路由合约。
4. 解释用户实际收到了多少资产，以及和预期价格的差距。
5. 写出如果让 AI Agent 执行同类交易，必须设置哪些限制。

## 扩展阅读

- [Ethereum DeFi](https://ethereum.org/en/defi/)：从用户视角理解 DeFi 的基础概念和风险。
- [ERC-20 Token Standard](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/)：理解同质化 token 的基础接口。
- [Uniswap Docs](https://developers.uniswap.org/docs/get-started/concepts/how-uniswap-works)：学习 AMM 和流动性池如何工作。
- [Aave Developers](https://aave.com/docs/developers/overview)：了解借贷协议的开发者入口和核心概念。
- [Chainlink Data Feeds](https://docs.chain.link/data-feeds)：理解 DeFi 中价格数据如何进入链上系统。
