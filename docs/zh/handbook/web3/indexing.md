...
::: info 相关 topic
- [智能合约（Smart Contract）](/zh/handbook/web3/smart-contract/)：先理解 event 为什么是合约向外部系统留下的日志。
:::
...
::: info 相关 topic
- [The Graph Subgraphs](https://thegraph.com/docs/en/subgraphs/overview/)：了解 subgraph、schema、mapping 和 GraphQL 查询。
:::
...
1. 选择一个简单合约，例如投票、计数器或 NFT mint。
2. 列出它应该发出的 event。
3. 设计一张查询表，例如 `votes`、`transfers` 或 `mints`。
...
- [Ethereum Events and Logs](https://ethereum.org/en/developers/docs/apis/backend/#events) ：理解后端如何监听链上事件。
- [The Graph Subgraphs](https://thegraph.com/docs/en/subgraphs/overview/)：学习 subgraph 的 schema、mapping 和查询方式。
