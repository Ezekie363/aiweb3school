...
::: info 相关 topic
- [Chainlink Data Feeds](https://docs.chain.link/data-feeds)：学习价格 feed 的使用方式、更新机制和链上接口。
:::
...
1. 找一个链上的 ETH/USD 或 BTC/USD price feed。
2. 查看 feed 地址、decimals、最新价格和更新时间。
3. 思考如果这个价格延迟 30 分钟，哪些协议动作会受影响。
4. 写出合约读取这个 feed 时应该检查哪些条件。
5. 再设计一个 AI Oracle 输出场景，列出必须记录的输入、模型版本和争议流程。

## 扩展阅读

- [Ethereum Oracles](https://ethereum.org/en/developers/docs/oracles/)：理解预言机为什么存在，以及链外数据如何进入链上。
- [Chainlink Data Feeds](https://docs.chain.link/data-feeds)：学习价格和数据 feed 的开发者使用方式。
- [Chainlink Data Feeds API Reference](https://docs.chain.link/data-feeds/api-reference)：查看 AggregatorV3Interface 等常用接口。
- [Pyth Docs](https://docs.pyth.network/)：了解另一类低延迟价格预言机网络。
- [UMA Optimistic Oracle](https://docs.uma.xyz/developers/optimistic-oracle)：学习带挑战机制的 optimistic oracle 思路。
